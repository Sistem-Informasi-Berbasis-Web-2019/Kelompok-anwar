...
$pdf->Cell(0,0.7,'Laporan Data Penjualan Buku',0,0,'C');
$pdf->ln(1);
...
$no=1;
	$qu = "SELECT * from lap_penjualan order by id_penjualan";
   $no=1;
	...
	$no++;
endforeach;
$prepare = $con->prepare("SELECT sum(total) as total from lap_penjualan");
            $prepare->execute();
...
$prepare = $con->prepare("SELECT sum(laba) as total_laba from lap_penjualan");
            $prepare->execute();
